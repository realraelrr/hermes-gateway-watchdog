...
derive_watchdog_seed_paths() {
  local seed_watchdog_home

  seed_watchdog_home="${WATCHDOG_HOME:-$HOME/.hermes-watchdog}"
...
derive_watchdog_paths() {
  ...
  : "${WATCHDOG_HOME:=$HOME/.hermes-watchdog}"
...
load_watchdog_env_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      HERMES_HOME|WATCHDOG_HOME|WATCHDOG_ENV_FILE|WATCHDOG_STATE_DIR|WATCHDOG_LOG_DIR|STATE_FILE|WATCHDOG_DISABLE_FILE|WATCHDOG_LOCK_DIR|WATCHDOG_RUNTIME_TMP_DIR|LOG_FILE|GATEWAY_STATE_FILE|WATCHDOG_ENABLED|FAIL_THRESHOLD|COOLDOWN_SEC|POST_RESTART_RETRIES|POST_RESTART_SLEEP_SEC|INITIAL_GRACE_SEC|TRANSITION_GRACE_SEC|LOCK_STALE_SEC|GATEWAY_LABEL|CLOUDFLARED_LABEL|CLOUDFLARED_READY_URL|FEISHU_WEBHOOK_PROBE_URL|GATEWAY_STATE_MAX_AGE_SEC|CURL_BIN|JQ_BIN|LAUNCHCTL_BIN|LSOF_BIN|NOTIFIER|DISCORD_WATCHDOG_WEBHOOK_URL|FEISHU_WATCHDOG_WEBHOOK_URL|DISCORD_WEBHOOK_URL|FEISHU_WEBHOOK_URL)
...
load_watchdog_config() {
  ...
  FEISHU_WEBHOOK_URL="${FEISHU_WEBHOOK_URL:-${FEISHU_WATCHDOG_WEBHOOK_URL:-}}"
}
